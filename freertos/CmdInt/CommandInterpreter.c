...
const signed char *pcRegisteredCommandString;
	...
	if( pxCommand == NULL )
	{
		...
		for( pxCommand = &xRegisteredCommands; pxCommand != NULL; pxCommand = pxCommand->pxNext )
		{
			...
			if( strncmppgm2ram( pcCommandInput, pcRegisteredCommandString, strlenpgm( pcRegisteredCommandString ) ) == 0 )
			{
	...
	if( ( pxCommand != NULL ) && ( xReturn == pdFALSE ) )
	{
		...
		was incorrect. */
		strncpypgm2ram( ( char * ) pcWriteBuffer, "Incorrect command parameter(s)\r\n", xWriteBufferLen );
		pxCommand = NULL;
	}
	...
	else
	{
		...
		strncpypgm2ram( pcWriteBuffer, "Command not recognised\r\n", xWriteBufferLen );
		xReturn = pdFALSE;
	}
...
signed portBASE_TYPE xReturn;
	...
	strncpypgm2ram( ( char * ) pcWriteBuffer, ( const char * ) pxCommand->pxCommandLineDefinition->pcHelpString, xWriteBufferLen );
